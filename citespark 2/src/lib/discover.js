...
async function fetchHtml(domain) {
  ...
  for (const url of attempts) {
    try {
      ...
      if (html.length > 200) return { html, finalUrl: res.url };
    } catch {
      // try the next form
    }
  }
...
export async function discoverSite(domainInput) {
  ...
  if (!fetched) {
    return {
      ok: false,
      domain,
      error: 'Could not reach that site. Check the domain, or fill the fields in by hand.'
    };
  }

  const signals = readSignals(fetched.html);
  ...
  if (!parsed || typeof parsed.brandName !== 'string') {
    return { ok: true, domain, source: 'page', confident: false, ...fallback, signals: summarise(signals) };
  }
