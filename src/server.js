...
/**
 * Search demand per cluster: real impressions and clicks beside the AI named
 * rate. Sources are the search consoles, the only place impressions exist.
 * Google today; Bing joins the same shape when connected, which is why the
 * response carries a source list rather than assuming one console.
 */
app.get('/api/projects/:id/demand', requireAuth, wrap(async (req, res) => {
  const project = await assertProject(req, res);
  if (!project) return;

  const { demandByCluster, DEMAND_METHOD, DEMAND_WINDOW_DAYS } = await import('./lib/demand.js');
  const sources = [];
  let queries = [];
  ...
  if (project.gsc_site_url && gscAuth(project).connected) {
    try {
      ...
      const rows = await fetchQueries(project, { days: DEMAND_WINDOW_DAYS });
      queries = rows.map((r) => ({ ...r, source: 'google' }));
      sources.push({ name: 'Google Search Console', queries: rows.length, ok: true });
    } catch (err) {
      // A console that failed is not a console that reported nothing.
      sources.push({ name: 'Google Search Console', ok: false, error: err.message });
    }
  } else {
    sources.push({ name: 'Google Search Console', ok: false, error: 'not connected' });
  }

  const rows = queries.length ? await demandByCluster(project.id, queries) : [];
  res.json({ rows, sources, method: DEMAND_METHOD, windowDays: DEMAND_WINDOW_DAYS });
}));
...
app.get('/api/prompts/:promptId/answers', requireAuth, wrap(async (req, res) => {
   ...
   *
   * The panel is the evidence view, and a domain is not evidence: hsbc.ae
   * could be a mortgage calculator or a wealth report, and the reader cannot
   * open a domain to check which. The url column has been stored all along;
   * it was simply never returned here.
   */
  const cites = runs.length
    ? await many(
        `SELECT run_id, domain, url FROM citations WHERE run_id = ANY($1) ORDER BY id`,
        [runs.map((r) => r.id)]
      )
    : [];
 ...
 */
/**
 * The brief that wins this answer, as pastable text.
 *
 * Deterministic and free: no model is called, so two requests for the same
 * question produce the same brief and generating one costs nothing. All the
 * specificity comes from measurement the project already paid for - the
 * engines' verdicts and the exact pages cited instead of the brand.
 */
/**
 * Where the money goes, and what turning each part off would save.
 *
 * The header showed one total after the fact. Nothing connected the figure
 * to its causes, and nothing let the person act on it: the levers existed
 * (engines, questions, runs) but lived three screens from the number they
 * move. This returns last cycle's spend split by engine and by question
 * intent, each with its named rate beside its cost, because "problem
 * questions cost $2.10 a cycle and were named in 2% of answers" is a
 * decision, while "$21.10" is only a fact.
 */
app.get('/api/projects/:id/spend', requireAuth, wrap(async (req, res) => {
  ...
  const byEngine = cycle ? await many(
    ...
    [project.id, owned?.id || -1, cycle]
  ) : [];

  const byIntent = cycle ? await many(
    `SELECT p.intent,
            COUNT(DISTINCT p.id) FILTER (WHERE p.active)::int AS active_questions,
  ...
  res.json({
    ...
    engines: byEngine.map((e) => ({
      ...e,
      enabled: (project.engines || []).includes(e.engine),
      perCall: pricing.costs?.[e.engine] ?? (e.calls ? e.spend / e.calls : 0)
    })),
    intents: byIntent,
    enabledEngines: project.engines || [],
    runsPerCycle: project.runs_per_cycle,
    perCall: pricing.costs || {},
    cap: Number(process.env.MAX_CYCLE_COST_USD || 15)
  });
}));

/**
 * Pause or resume every question of one intent, as one deliberate act.
 *
 * The audit found problem-intent questions named in 2% of answers while
...
app.patch('/api/projects/:id/intents', requireAuth, wrap(async (req, res) => {
  const project = await assertProject(req);
  if (!project) return res.status(404).json({ error: 'Not found' });
  const { intent, active } = req.body || {};
  if (typeof intent !== 'string' || typeof active !== 'boolean') {
    return res.status(400).json({ error: 'Send intent and active' });
  }
  const out = await many(
    `UPDATE prompts SET active = $3 WHERE project_id = $1 AND intent = $2 RETURNING id`,
    [project.id, intent, active]
  );
  res.json({ ok: true, changed: out.length, active });
}));

app.get('/api/prompts/:promptId/brief', requireAuth, wrap(async (req, res) => {
  const prompt = await one(
    `SELECT p.*, per.name AS persona_name
     FROM prompts p
  ...
  if (cycle && owned) {
    const rows = await many(
      `SELECT r.id, r.engine, m.mentioned, r.response_text
       ...
       ORDER BY r.engine, r.run_index`,
      [prompt.id, cycle, owned.id]
    );
    const cites = rows.length
      ? await many('SELECT run_id, domain, url FROM citations WHERE run_id = ANY($1) ORDER BY id', [rows.map((r) => r.id)])
      : [];
    const byRun = new Map();
    for (const c of cites) {
    ...
    // One line per engine: several samples collapse to "named anywhere",
    // and their citations merge, because the brief needs who owns the
  ...
  const siblings = await many(
    `SELECT text FROM prompts
     WHERE project_id = $1 AND cluster = $2 AND id <> $3 AND active
     ORDER BY ai_search_volume DESC LIMIT 5`,
    [project.id, prompt.cluster, prompt.id]
  );

  const { buildBrief } = await import('./lib/brief.js');
  res.type('text/plain').send(buildBrief({
    project,
    prompt,
